...
uniform float uTime; // Pass elapsed time from Haxe (in seconds)
...
void main()
{
    ...
    pixel_color.r = mix(
        pixel_color.r,
        texture2D(bitmap, fixed_uv + vec2(shake_color_rate, 0.0)).r,
        enable_shift
    );

    pixel_color.b = mix(
        pixel_color.b,
        texture2D(bitmap, fixed_uv + vec2(-shake_color_rate, 0.0)).b,
        enable_shift
    );
